...
Докажем, что $\inf\{r\}=-\sqrt2$ \newline 1) $-\sqrt2<r$, для $\forall r \in \{r\}$
\newline 2) $\forall \epsilon \exists r^{''} \in \{r\}: r^{''} < -\sqrt 2 + \epsilon $, т.к. между двумя произвольными действительными числами всегда найдется рациональное число. 
